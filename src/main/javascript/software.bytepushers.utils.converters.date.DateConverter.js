...
VA_AMS.converters.DateConverter.convertToString_YYYYMMDDThhmmsssTZD = function (d, delimeter) {
    "use strict";
    delimeter = (Object.isDefined(delimeter)) ? delimeter : "";
    if(!Object.isDate(d)) throw Error("parameter d must be a Date Object.");

    //TODO: need to refactor this to create the right date format.
    //return d.getFullYear() + delimeter + VA_AMS.NumberUtility.padLeft(d.getMonth()+1, 1)+ delimeter + d.getDate() + "T" + ;
...
VA_AMS.converters.DateConverter.convertToString = function (d, dateFormat, delimeter) {
    ...
    switch (dateFormat) {
        case VA_AMS.converters.DateConverter.YYYYMMDD_DATE_FORMAT:
            ...
            break;
        case VA_AMS.converters.DateConverter.YYYYMMDDThhmmsssTZD_DATE_FORMAT:
            date = VA_AMS.converters.DateConverter.convertToString_YYYYMMDDThhmmsssTZD(d, delimeter);
    }
    return date;
};
